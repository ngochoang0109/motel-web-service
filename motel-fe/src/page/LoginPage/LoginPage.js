...
const LoginPage = () => {
    ...
    const [routeCheck, setRouteCheck] = useState(0);
    ...
    useEffect(() => {
        ...
        if (!mounted.current) {
            ...
            if (auth === true) {
                if (userInfor.role.length !== 0) {
                    console.log(userInfor)
                    for (let i = 0; i < userInfor.role.length; i++) {
                        if (userInfor.role[i].name === 'ROLE_ADMIN') {
