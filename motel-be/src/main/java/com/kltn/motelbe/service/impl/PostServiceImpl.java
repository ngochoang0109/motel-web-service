...
public class PostServiceImpl implements PostService {
	...
	public PostResponse savePost(PostRequest postRequest, List<MultipartFile> images, List<MultipartFile> videos,
		...
		if (images != null) {
			...
			Set<ImageDto> lstImages = imageService.saveImages(imageMapper.convertUrlImagesToImages(urlImages),
					newPost.getAccommodation());
			...
			return new PostResponse(newPost.getId(), img, newPost.getTitle(), newPost.getAccommodation().getPrice(),
	...
	public Paging<PostResponse> getPostsOfUser(int pageNo, int pageSize, String field, String username) {
		...
		Paging<PostResponse> paging = new Paging<>(postMapper.mapPostsToPostResponses(posts.getContent()),
				posts.getNumber(), posts.getSize(), posts.getTotalElements(), posts.getTotalPages(), posts.isLast(),
				posts.isFirst());
		return paging;
	}
	...
	public Paging<PostResponse> getPosts(int pageNo, int pageSize, String field) {
		...
		List<PostResponse> postResponses = postMapper.mapPostsToPostResponses(posts.getContent());

		Collections.sort(postResponses, new Comparator<PostResponse>() {
			@Override
			public int compare(PostResponse obj1, PostResponse obj2) {
				return obj2.getCreatedDate().compareTo(obj1.getCreatedDate());
			}
		});
		
		Paging<PostResponse> paging = new Paging<>(postResponses,
				posts.getNumber(), posts.getSize(), posts.getTotalElements(), posts.getTotalPages(), posts.isLast(),
				posts.isFirst());
	...
	/* output: get k new least post is approved and after is createDate */
	@Override
	public List<PostResponse> getTopKLeastNewPost(int k, String field, long id) {
		Pageable pageable = PageAndSortUtils.getPageable(0, k, field);
		Page<Post> posts = postRepository.getPosts(pageable);
		List<PostResponse> postResponses = postMapper.mapPostsToPostResponses(posts.getContent());
		...
		return postResponses.stream().filter(item -> item.getId() != id).collect(Collectors.toList());
	}

	/*
	 * output: get k related post follow 2 criteria are: + same ward and province +
	 * same type of post is sorted by approved time
	 */
	@Override
	public List<PostResponse> getRelatedPosts(String address, long typePost, long id, int k, String field) {
		Pageable pageable = PageAndSortUtils.getPageable(0, k, field);
		Page<Post> posts = postRepository.getRelatedPosts(pageable, typePost, address, id);
		List<PostResponse> postResponses = postMapper.mapPostsToPostResponses(posts.getContent());
	...
	public Paging<PostResponse> getPostsOfType(String shortName, int pageNo, int pageSize, String field) {
		Pageable pageable = PageAndSortUtils.getPageable(pageNo, pageSize, field);
		Page<Post> posts = postRepository.getPostsByType(pageable, shortName);
		List<PostResponse> postResponses = postMapper.mapPostsToPostResponses(posts.getContent());
